import os
from os import getenv


# ------------------------------------------------
API_ID = int(os.environ.get("API_ID", "34422904"))
# ------------------------------------------------
API_HASH = os.environ.get("API_HASH","7e0002469784f47fc08a6b3d93d7ebed")
# ------------------------------------------------
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8684246543:AAHSspI4eNVh6IikZ9uCbNt6j-J-nWcFjuA")
# ------------------------------------------------
BOT_USERNAME = os.environ.get("BOT_USERNAME", "@Helpbykrishna2_botbot")
BOT_TEXT = ":𝐈𝐓'𝐬𝐆𝐎𝐋𝐔.™®:"
# ------------------------------------------------
OWNER_ID = int(os.environ.get("OWNER_ID", "5349573682"))
# ------------------------------------------------
# //LOG CHANNEL ID 
CHANNEL_ID = int(os.environ.get("CHANNEL_ID", "-1003871588534"))

# //FORCE_CHANNEL_ID
CHANNEL_ID2 = int(os.environ.get("CHANNEL_ID2", "-1003871588534")) 
# ------------------------------------------------
MONGO_URL = os.environ.get("MONGO_URL", "mongodb+srv://adarshppandey937:uIoPcln9vXQBF0vP@cluster0.o9mn6hb.mongodb.net/?")
# -----------------------------------------------
PREMIUM_LOGS = int(os.environ.get("PREMIUM_LOGS", "-1003871588534"))
# -----------------------------------------------
join = '<a href="">✳️ JOIN BACKUP</a>'
# -----------------------------------------------
UNSPLASH_ACCESS_KEY = 'RabDRmuXXBobanmwwbvpP5LwoG4J8ox34y5Sstz-9jk'
# -----------------------------------------------
UNSPLASH_QUERY = 'animal baby'
# -----------------------------------------------
ADMIN_BOT_USERNAME = "Helpbykrishna1_bot" #without @

THUMB_URL = os.environ.get("THUMB_URL", "https://ibb.co/hpwD5ZZ")




# # Bot configuration
# API_ID = int(os.environ.get("API_ID", "34422904"))
# API_HASH = os.environ.get("API_HASH", "7e0002469784f47fc08a6b3d93d7ebed")
# BOT_TOKEN = os.environ.get("BOT_TOKEN", "8684246543:AAHSspI4eNVh6IikZ9uCbNt6j-J-nWcFjuA")
# BOT_USERNAME = os.environ.get("BOT_USERNAME", "Helpbykrishna2_botbot")
# OWNER_ID = int(os.environ.get("OWNER_ID", "5349573682"))
# SUDO_USERS = list(map(int, getenv("SUDO_USERS", "7463601722").split()))
# CHANNEL_ID = int(os.environ.get("CHANNEL_ID", "-1003871588534"))
# MONGO_URL = os.environ.get("MONGO_URL", "mongodb+srv://wadiro6523:08AwfhhKRdQaS1i6@cluster0.krzxuop.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
# PREMIUM_LOGS = int(os.environ.get("PREMIUM_LOGS", "-1003871588534"))
# THUMB_URL = os.environ.get("THUMB_URL", "https://i.fbcd.co/products/original/ug-circle-logo-design-2-e84695ca2ab9a697d2b2d7c928b0bf5f12bf18e076da241815e0372c8d617915.jpg")

